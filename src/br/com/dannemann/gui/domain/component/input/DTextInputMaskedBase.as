package br.com.dannemann.gui.domain.component.input
{
	 ...
	 * 	@mxml
	 *
	 * 	<p>The <code>&lt;mx:DTextInput&gt;</code> tag inherits the attributes
	 ...
	 *  </pre>
	 *
	 *  @includeExample ../../../../../../docs/com/adobe/flex/extras/controls/example/DTextInputExample/DTextInputExample.mxml
	 *
	 ...
	 *
	 */
	// This class must be public otherwise the component will not work properly.
	public class DTextInputMaskedBase extends TextInput
	{
		public var _width:int = 108; // INFO: Adicionei esta linha.
		public var _percentWidth:int = 0; // INFO: Adicionei esta linha.
		 ...
		 *  Constructor.
		 */
		public function DTextInputMaskedBase()
		{
			super();
		...
		override public function get text():String
		{
			...
			var result:String = "";
			var worlen:int = _working.length;
			for(var i:int=0; i < worlen; i++) {
		...
		private function calculateMaxChars():Number
		{
			maskMap = new Array();
			var count:Number = 0;
			var inpMaslen:int = _inputMask.length;
			for(var i:int=0,k:int=0;i<inpMaslen;i++,k++)
			{
		...
		private function interceptChar( event:TextEvent ) : void
		{
			...
			if( _position >= actualLength ) {
				...
				if(_autoAdvance && !isWorkingIncomplete())
				{
					...
					if((obj is DTextInputMaskedBase) || (obj is TextInput) || (obj is TextArea))
						obj.setFocus();
				}
		...
		private function interceptFocus( event:FocusEvent ) : void
		{
			...
			else if(event.type == FocusEvent.FOCUS_OUT)
			{
				...
				// INFO: COMMENTED BY ME AS I DON'T WANT ANY COLOR CHANGE IN THIS COMPONENT.
				//		 THIS WAS CAUSING A BUG AFTER PRESSING THE "CLEAN/NEW" BUTTON ON THE CRUD TOOLBAR.
				//		 THE TEXT WAS GETTING RED AFTER THE FOCUS WAS OUT OF THE COMPONENT.
...
//				}
		...
		{
			if (input) // INFO: Adicionei este "if".
			{
				var inplen:int = input.length;
				for(var i:int = 0;i<inplen;i++)
				{
					...
					switch(m)
					{
						...
						case "B":
							...
							break;
						// Retirei o parâmetro de máscara ".".
						/*case ".":
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if(fontUpdated)
			{
				...
				{
					setStyle("fontFamily","Verdana"); // INFO: Modifiquei de Courier para Verdana.
				}
			...
			//createSkin(); INFO: Commented by me.

			//textField.width += 50; INFO: Commented by me.

			// INFO: I added this condition.
			if (_percentWidth > 0)
		...
		private function createSkin():void
		{
			...
			if (!newSkin)
			{
				...
				if (!newSkinClass)
				{
					//newSkinClass = MTISkin; INFO: COMENTEI ESTA LINHA POIS DELETEI MTISkin.
				}
				if(newSkinClass)
				{
